...
Import OffsetFault and other packages that are used in this tutorial
...
![original](../../images/demos/offset_fault/original.png)

## Define Fault

This example will utilize the `OffsetFault` library to create an instance of the normal noise fault. The `OffsetFault` expects the `params` variable to be equal to a dictionary containing the key `offset_by`. This corresponds to the value that will be added to the true values.

```python
fault_params = {"offset_by": 0.1}
fault_obj = OffsetFault(params=fault_params)
```
...
![delta](../../images/demos/offset_fault/delta.png)
...
![comparison](../../images/demos/offset_fault/comparison.png)
