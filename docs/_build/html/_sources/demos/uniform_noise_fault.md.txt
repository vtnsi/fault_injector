...
Import UniformNoiseFault and other packages that are used in this tutorial
...
![original](../../images/demos/uniform_noise_fault/original.png)

## Define Fault

This example will utilize the `UniformNoiseFault` library to create an instance of the uniform noise fault. The `UniformNoiseFault` expects the `params` variable to be equal to a dictionary containing the keys `min_val` and `max_val`.

For more information as to how `min_val` and `max_val` are used, please see the `low` and `high` variable in the Numpy random.uniform documentation, [found here](https://numpy.org/doc/stable/reference/random/generated/numpy.random.Generator.uniform.html)

```python
fault_params = {"min_val": 0,
                "max_val": 0.1}
fault_obj = UniformNoiseFault(params=fault_params)
```
...
![delta](../../images/demos/uniform_noise_fault/delta.png)
...
![comparison](../../images/demos/uniform_noise_fault/comparison.png)
